...
function MMI.fit(model::SumRatioHSE, verbosity, X, y)
    ...
    G = y.arrays[y.summaries[treatmentnames[2]].matrix]
...
function MMI.predict(model::SumRatioHSE, fitresult, Xy_nu, Xy_de) 
    ...
    G_nu = Xy_nu.arrays[Xy_nu.summaries[fitresult.treatmentnames[2]].matrix]
    ...
    G_de = Xy_de.arrays[Xy_de.summaries[fitresult.treatmentnames[2]].matrix]
